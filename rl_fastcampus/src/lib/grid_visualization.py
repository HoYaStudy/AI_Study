...
def visualize_value_function(
    ax, v_pi: np.array, nx: int, ny: int, plot_cbar=True  # matplotlib axes object
):
    hmap = ax.imshow(v_pi.reshape(nx, ny), interpolation="nearest")
    if plot_cbar:
...
def visualize_policy(ax, pi: np.array, nx: int, ny: int):
    ...
    ax.set_xticks(np.arange(pi.shape[1] + 1) - 0.5, minor=True)
    ax.set_yticks(np.arange(pi.shape[0] + 1) - 0.5, minor=True)
    ax.set_xticklabels([])
    ax.set_yticklabels([])
    ...
    for i in range(ny):
        for j in range(nx):
            ...
            ax.text(
                i, j, direction, ha="center", va="center", color="black", fontsize=20
            )
