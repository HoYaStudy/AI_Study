...
class ExactMCAgent:
    ...
    def __init__(self, gamma: float, num_states: int, num_actions: int, epsilon: float):
        self.gamma = gamma
        ...
        self._eps = (
            1e-10  # for stable computation of V and Q. NOT the one for e-greedy !
        )
...
class MCAgent(ExactMCAgent):
    ...
    def __init__(
        self, gamma: float, num_states: int, num_actions: int, epsilon: float, lr: float
    ):
        super(MCAgent, self).__init__(
            gamma=gamma, num_states=num_states, num_actions=num_actions, epsilon=epsilon
        )
        self.lr = lr
...
if __name__ == "__main__":
    ...
    mc_agent = MCAgent(
        gamma=1.0, lr=1e-3, num_states=nx * ny, num_actions=4, epsilon=1.0
    )
